...
public:
    ...
    bool GetMembers(const std::vector<std::string> &names, int index, rapidjson::Value &jsonValue, rapidjson::Document::AllocatorType &allocator, TYPE &arg)
    {
        ...
        rapidjson::Value key;
        key.SetString(names[index].c_str(), (rapidjson::SizeType)names[index].length(), allocator);
...
public:
    ...
    {
        char *end;
        obj = (unsigned int)strtoul(value.c_str(), &end, 10);
    }
...
public:
    ...
    bool JsonToObject(std::vector<TYPE> &obj, rapidjson::Value &jsonValue)
    {
        ...
        rapidjson::SizeType arraySize = (rapidjson::SizeType)array.Size();
...
public:
    ...
    {
        jsonValue.SetString(obj.c_str(), (rapidjson::SizeType)obj.length(), allocator);
        return true;
    }
