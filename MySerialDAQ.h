#pragma once
#include <atomic>
#include <fstream>
#include <iostream>
#include <string>
#include "AIGCJson/AIGCJson.hpp"
#include "picojson.h"
#include "JsonFileModel.h"
#include <errno.h>
#include <termios.h>
#include <sys/stat.h>
#include <sys/time.h>
#include <fcntl.h>
#include <unistd.h>

#include "MyTimer.h"
//#include "modbus/modbus-private.h"
//#include "modbus//modbus.h"
//#include "modbus/modbus-rtu.h"

using namespace std;
using namespace aigc;

//typedef unsigned char byte;
#define MAXBUF  255
#define MAX_RTU_NUM_PER_SERIAL_PORT 100

#ifdef __cplusplus
extern "C" {
#endif

extern int usrMasterServiceStart(void);

#ifdef __cplusplus
}
#endif

class MySerialDAQ
{
public:
	MySerialDAQ(int portNum);

	int getPortNum() {  return portnum; }

    void start();


    thread thread_start()
    {
        return thread(&MySerialDAQ::start, this);
    }

    //std::thread member2Thread(const char* arg1, unsigned arg2) {
    //    return std::thread(&Wrapper::member2, this, arg1, arg2);
    //    w->member2Thread("hello", 100).detach();
    //https://www.cnblogs.com/c4isr/p/9205164.html

    void stop() { isCanceled = true; }

    int readConfFile();

    int set_serial(int fd, int nSpeed, int nBits, char nEvent, int nStop);


    //check existing of port configuration file
    static bool conf_file_exist(string fileName)
    {
        return (access(fileName.c_str(), F_OK) != -1);
    }

    int readRev(char* revBuf);

    //add write command to the queue
    int add_cmd_to_queue(string cmd);

    string GetReadCommand(int addr, int functionCode, int regStart, int regNum);

    int get_length_by_datatype(int code, int datatype);

    unsigned int crc16(unsigned char* updata, unsigned int len);


    //byte array to string
    string byte_array_to_string(char* key, int length);

    //string to byte pointer
    //byte* px = (byte*)str.c_str();
    //int len = str.length();

    //float to unsigned char array
    unsigned char * bytes_to_float(float f)
    {
        unsigned char* p = (unsigned char*)&f;
        return p;
    }

    bool validate_return_data(int stationNum, int functionCode, int regNum, char* buf, int len);

    bool parse_return_data(char* buf, int len)
    {
        return true;
    }

    /*
    *function：float ByteToFloat(unsigned char* byteArray)
    *decription:  将字节型转化成32bits浮点型
    *input:长度为4的字节数组
    *output:
    */
    float ByteToFloat(uint8_t* byteArray)
    {
        return *((float*)byteArray);
    }

    double ByteToDouble(uint8_t* byteArray)
    {
        return *((double*)byteArray);
    }

    long long BCD_TO_BIN64(long long bcd);

private:
	int portnum = 0;
    string strJsonFileName("");
    Modbus_RtuSerialPortConf conf = NULL;
    int port_fd = -1;
    int collectPeriod = 0;
    atomic<bool> isCanceled;
    MyTimer timer;
    //modbus_t* ctx = NULL;
    string strPort;
    vector<string> vecControls;
    mutex mutexWrite;
    unsigned char auchCRCHi[256] =
    {
     
    };
    unsigned char auchCRCLo[256] =
    {
     
    };
};

thread usrSerialCollectStart(int port);
